...
Storage.prototype.removeCacheItem = function (key) {
    ...
    if (expireDate && now <= expireDate) {
        ...
        try {
            ...
            if (parsed.isJson) {
                ...
                return parsed; // return the string, since it could be trying to do JSON.parse("3") which will succeed and not throw an error, but "3" isn't a json obj
            }
        } catch (e) {
            return value;
        }
...
function setExpirationSession(key, expireDate) {
    var expirations = sessionStorage.getItem("sessionStorageExpirations"); // "key1^11/18/2011 5pm|key2^3/10/2012 3pm"
    if (expirations) {
        var arr = expirations.split("|"); // ["key1^11/18/2011 5pm","key2^3/10/2012 3pm"]
        for (var i = 0; i < arr.length; i++) {
            var expiration = arr[i]; // "key1^11/18/2011 5pm"
        ...
        arr.push(key + "^" + expireDate.toString());
        sessionStorage.setItem("sessionStorageExpirations", arr.join("|"));
    } else {
        sessionStorage.setItem("sessionStorageExpirations", key + "^" + expireDate.toString()); // "favColor^11/18/2011 5pm etc etc"
    }
}

function getExpirationSession(key) {
    var expirations = sessionStorage.getItem("sessionStorageExpirations"); // "key1^11/18/2011 5pm|key2^3/10/2012 3pm"
    if (expirations) {
        var arr = expirations.split("|"); // ["key1^11/18/2011 5pm","key2^3/10/2012 3pm"]
        for (var i = 0; i < arr.length; i++) {
            ...
            if (k == key) { // found match; return expiration and remove expiration if it's expired
                ...
                if (now > eDate) {
                    ...
                    if (arr.length > 0) {
                        ...
                        sessionStorage.removeItem("sessionStorageExpirations");
                    }
                }
                return new Date(e);
            }
...
// ex: sessionStorage.setCacheItem("favColor", "blue", { days: 1 })
Storage.prototype.setCacheItemSession = function (key, value, exp) {
    var val = null;
    if (typeof value == 'object') {
        // assume json
        value.isJson = true; // add this flag, so we can check it on retrieval
    ...
    setExpirationSession(key, expireDate);
};

Storage.prototype.removeCacheItemSession = function (key) {
    // TODO: return JSON.parse if value is stringify'd json obj

    // first, check to see if this key is in sessionstorage
...
Storage.prototype.getCacheItemSession = function (key) {
    // TODO: return JSON.parse if value is stringify'd json obj
