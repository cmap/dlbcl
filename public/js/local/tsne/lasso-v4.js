// Original: http://axc.net/code_libraries/lasso/lasso.js
...
d3.lasso = function() {
	...
	function lasso(s) {
		...
		function dragstart() {
            ...
            var itemss = [];
            [].forEach.call(items._groups[0], function(i) {
                itemss.push(i);
            });
            itemss.forEach(function(d) {
			...
			if(hoverSelect==true) {
				itemss.on("mouseover.lasso",function() {
		...
		function dragmove() {
			...
			isPathClosed = distance<=closePathDistance ? true : false;
	  		...
	  		var complete_path_d = d3.select("path")._groups[0][0].attributes.d.value + "Z";
			 ...
			 if(isPathClosed == true && closePathSelect == true) {
			 	close_path.attr("d",close_draw_path);
			  	...
			  	for (var i = 0; i<=close_path_length; i++) {
			  		...
			  		itemss.filter(function(d) {return d.lassoPoint.cy==Math.round(cur_pos.y)}).forEach(function(d) {
			...
			// Tag possible items
			d3.selectAll(itemss.filter(function(d) {return (d.loopSelected && isPathClosed) || d.hoverSelected}))
